...
и не все совместимы с чистым Рефалом-5.
...
### Функции `TryLoadExpr` и `LoadExpr`
...
Функции принимают имя файла, в котором записано объектное выражение
в синтаксисе Рефала-5 и разбирают его. В выражении допустимы оба вида
комментариев: однострочные на «`*`» и многострочные «`/* … */`».

При отсутствии синтаксических ошибок функция `TryLoadExpr` возвращает `Success`
и прочитанное выражение, иначе `Fails` и сообщение о первой синтаксической
ошибке.

Функция `LoadExpr` при наличии синтаксических ошибок выводит сообщение о первой
ошибке на `stderr` в формате `‹имяфайла›:‹строка›:‹колонка›:‹сообщение›`
и завершает программу вызовом `<Exit 1>`. Реализована она как обёртка над
`TryLoadExpr`.
...
**Примечание.** Рефал-5λ поддерживает функции высшего порядка — указатели
на функциии и безымянные функции («лямбды»), их тоже можно вызывать посредством
функции `Mu`, поэтому их тоже можно использовать в упомянутых ниже функциях.

В Рефале-5λ тоже есть библиотека `LibraryEx` и она совместима с `LibraryEx`
настоящего набора библиотек, однако имеет некоторые расширения (не описанные
здесь функции). Рекомендуем обратиться к библиотеке или исходникам `LibraryEx`
Рефала-5λ для получения дополнительных подробностей.
